...
try:
    ...
    # 2. click select 1 bedrooms
    bedroom1_buttons = driver.find_element(By.CSS_SELECTOR, 'button[data-bedrooms="1"]')
    bedroom1_buttons.click()
    time.sleep(3)


    total_value = driver.find_element(By.ID, "total-load-kw")
    assert "3.35"== total_value.text, "test total-kwh-value failed, expected 3.35, but got " + total_value.text
    ...
    for row in rows:
        ...
        if appliance_name == "Fan - Ceiling":
            row.find_element(By.CLASS_NAME, "quantity").click()
            row.find_element(By.CLASS_NAME, "quantity").send_keys("2")
            break
    time.sleep(5)
...
=======
    assert "3.79"== total_value.text, "test total-kwh-value failed, expected 3.79, but got " + total_value.text
>>>>>>> updated-test-by-zinan
